...
public class WitherVenenoSkill {

	private Skills skill = Skills.VENENO_DO_WITHER;
	//Numero de ticks do veneno
	private int venenoTicks = 45;
	//Porcentagem de chance de dar o veneno
	private int porcentagem = 25;
	public SmashMobs plugin;
	public WitherVenenoSkill(SmashMobs plugin){
		this.plugin = plugin;
	}
						...
						if(SkillEventUtil.playerHasSkill(player, skill)){
							...
							if(porcentagem <= rnd){
								if(e.getEntity() instanceof LivingEntity){
									((LivingEntity)e.getEntity()).addPotionEffect(new PotionEffect(PotionEffectType.WITHER, venenoTicks, 1));
