...
public class CustomDragao extends EntityEnderDragon implements CustomBoss{

	private Boss boss;
	private Explosion explosionSource = new Explosion(null, this, (0.0D / 0.0D), (0.0D / 0.0D), (0.0D / 0.0D), (0.0F / 0.0F));
	private Entity bD;

	public CustomDragao(World world, Boss boss)
	{
		super(world);
		this.boss = boss;
		this.fireProof = true;
	...
	private void b(List list)
	{
		for(int i = 0; i < list.size(); i++)
		{
			...
			if((entity instanceof EntityLiving))
			{
				...
				if(random <= 30)
				{
					((EntityLiving) entity).addEffect(new MobEffect(MobEffectList.CONFUSION.id, 8 * 20, 0));
				}
			}
		}
	}
